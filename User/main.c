...
void Send_Json_Data(char name[], u32 data, json_t *root, char *out);
void MLX_Init(void);
void Temp_show(void);
...
int main(void)
{
  ...
  {	
		...
		if(flog==1)
		{
			...
			// 数据：呼吸波、心电信号、心率
			flog=0;
			Send_Json_Data("huxi", (u32)Output_data1, root, out);
			Send_Json_Data("xindian", (u32)Output_data2, root, out);
			Send_Json_Data("xinlv", (u32)BPM, root, out);
		}
  }
}
...
void MLX_Init(void)
{
	while(!vMLX90614_Init(&sMlx,MLX90614_SMBUS_ADDR))
	{
		printf("MLX90614 Error!\r\n");
		printf("MLX90614 Error!");
		delay_ms(200);
	}
	printf("MLX90614 OK!\r\n");
...
void Temp_show(void)
{
	...
	Send_Json_Data("body",(u32)sMlx.To1, root, out);

}

void Send_Json_Data(char name[], u32 data, json_t *root, char *out){
	root = json_pack("{s:i}", name, data);	//构建json格式数据
